...
async function seedUser () {
  try {
    ...
    userToBeInserted = await dbService.findOne(model.user,{ 'nik':'jkzcjdoyre' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'MS8NfApjWxiC0dU',
        'isDeleted':false,
        'nik':'jkzcjdoyre',
        'email':'Deon_Heidenreich92@hotmail.com',
        'username':'Alvah_Baumbach32',
        'isActive':true,
      ...
      await dbService.update(model.user, { 'nik':'jkzcjdoyre' }, userToBeInserted);
    }
    userToBeInserted = await dbService.findOne(model.user,{ 'nik':'8g404mat65' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'LjCAVaSiY6_vdzU',
        'isDeleted':false,
        'nik':'8g404mat65',
        'email':'Marley97@yahoo.com',
        'username':'Skyla_Swift61',
        'isActive':true,
        'userType':authConstant.USER_TYPES.Staff
      };
      await dbService.createOne(model.user,userToBeInserted);
    } else {
      userToBeInserted = {
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await dbService.update(model.user, { 'nik':'8g404mat65' }, userToBeInserted);
    }
